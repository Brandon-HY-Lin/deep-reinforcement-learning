...
""" DDPG Agent (Version 4)
    1. Reset OUNoise after every calling learn()
    2. Add clip gradient
    3. Add prioritized experience replay (PER)
    4. Add batch normalization to critic and actor networks in each agent.
    5. Add exploration weight decay.
"""
class DDPGAgentVersion4(BaseAgent):
    def __init__(self, state_size, action_size, random_seed,
        ...
        self.actor_local = DDPGActorVersion2(state_size, action_size, random_seed, 
                                             fc1_units=fc1_units, fc2_units=fc2_units).to(device)
        
        self.actor_target = DDPGActorVersion2(state_size, action_size, random_seed, 
                                              fc1_units=fc1_units, fc2_units=fc2_units).to(device)

        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=lr_actor)
        
        # Critic Network (w/ Target Network)
        self.critic_local = DDPGCriticVersion2(state_size, action_size, random_seed, 
                                               fcs1_units=fc1_units, fc2_units=fc2_units).to(device)
        
        self.critic_target = DDPGCriticVersion2(state_size, action_size, random_seed, 
                                                fcs1_units=fc1_units, fc2_units=fc2_units).to(device)
